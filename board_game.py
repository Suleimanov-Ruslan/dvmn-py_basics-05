...
def generate_character():
    ...
    runic_skills = [''.join(MAPPING.get(char, char) for char in skill) for skill in skills]
...
def save_character_card(character_data, index):
    os.makedirs(OUTPUT_FOLDER, exist_ok=True)
    file_name = os.path.join(OUTPUT_FOLDER, f"result_{index}.svg")
    file_operations.render_template("charsheet.svg", file_name, character_data)


def main():
    for i in range(10):
        character = generate_character()
        save_character_card(character, i)
